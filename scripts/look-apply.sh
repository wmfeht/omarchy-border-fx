#!/usr/bin/env bash
# Fan-out: Omarchy plugin look (JSON, camelCase, rgba()) on qs.border-fx
...
lua=$(LOOK_JSON="$look_json" SESSION_SO="$SESSION_SO" SHINY_LOAD="$load" SHINY_ENABLED="$enabled" python3 - <<'PY'
...
shiny_effect = look.get("effect") == "shiny"
load = os.environ.get("SHINY_LOAD", "1") != "0" and shiny_effect
enabled = os.environ.get("SHINY_ENABLED", "1") != "0" and shiny_effect

lines = [
    "-- Generated by qs.border-fx (look-apply). Do not edit.",
    "-- Source of truth: ~/.config/omarchy/shell.json plugins[] entry id qs.border-fx.",
    "-- Effect " + lua_str(look["effect"]) + " fans out to the shiny Hyprland adapter (plugin:shiny-border / shiny_border) when effect is shiny.",
    "",
    ...
    "if SHINY_LOAD and not _G.__qs_border_fx_start then",
    "  _G.__qs_border_fx_start = true",
