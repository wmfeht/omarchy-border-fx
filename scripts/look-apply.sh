#!/usr/bin/env bash
# Fan-out: Omarchy plugin look (JSON, camelCase, rgba()) → generated
# ~/.config/hypr/shiny-border.lua and optional hyprctl eval.
...
--eval      hyprctl eval dofile() of that lua, only if hypr-shiny-border is loaded
...
load = os.environ.get("SHINY_LOAD", "1") != "0"
enabled = os.environ.get("SHINY_ENABLED", "1") != "0"

lines = [
    "-- Generated by qs.shiny-border (look-apply). Do not edit.",
    "-- Source of truth: ~/.config/omarchy/shell.json plugins[] entry id qs.shiny-border.",
    "",
    ...
    "if SHINY_LOAD and not _G.__qs_shiny_border_start then",
    "  _G.__qs_shiny_border_start = true",
